...
const HISTORY_ROWS: HistoryRow[] = [
  ...
  },
  {
    id: 'api-reference-en',
    icon: 'check',
    name: 'api-reference-en · generation',
    tag: 'full',
    target: '86 pages',
    started: 'yesterday',
    duration: '5:18',
  ...
  {
    id: 'faq-produit-de-export',
    icon: 'alert',
    name: 'faq-produit-de · export',
    tag: 'langfuse',
    target: '—',
    started: '3 d ago',
    duration: '0:08',
    ...
    <div className="jobs-page">
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">
            Scraping, generation and export pipelines running, queued or completed.
          </p>
        </div>
      ...
      <div className="tabs" style={{ marginBottom: 18 }}>
        <button className="tab active">
          Running <span className="muted">3</span>
        </button>
        <button className="tab">
          Queue <span className="muted">2</span>
        </button>
        <button className="tab">Completed</button>
        <button className="tab">
          Failed <span className="muted">1</span>
        </button>
      </div>
        ...
        <div className="job" key={job.id}>
          ...
          <div className="job-stages">
            {job.stages.map((stage) => (
              <div className={`stage${stage.state ? ` ${stage.state}` : ''}`} key={stage.label}>
                {stage.label}
              </div>
            ))}
          </div>
      ...
      <div className="card" style={{ marginTop: 22 }}>
        <div className="card-head">
          <div>
            <div className="card-title">Queue &amp; history</div>
          </div>
        </div>
          ...
          <tbody>
            {HISTORY_ROWS.map((row) => (
              <tr key={row.id}>
                ...
                <td>
                  <span className={`badge badge-${row.badgeVariant}`}>
                    <span className="dot" />
                    {row.status}
                  </span>
                </td>
