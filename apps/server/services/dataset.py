...
def analyze_dataset_similarities(
    ...
    try:
        ...
        if not dataset:
            ...
            raise ValueError(f"Dataset '{dataset_id}' not found. Available datasets: {available_datasets}")
        ...
        logging.error(f"Error analyzing similarities for dataset {dataset_id}: {str(e)}")
        raise
