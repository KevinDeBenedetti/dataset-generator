  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mb-4 break-words whitespace-pre-wrap">{dataset.description}</p>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2 text-xs text-gray-600">{qaResponse?.dataset_id ?? '-'}</span>
            </div>
          </div>
        </div>
        ...
        {versions.length > 0 && (
          <div className="mt-4 bg-gray-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium mb-2">Langfuse versions ({versions.length})</h3>
            <ul className="divide-y text-sm">
              {versions.map((v) => (
                ...
                >
                  <span className="font-mono font-medium">{v.run_name ?? `v${v.version}`}</span>
                  <span className="text-gray-600">{v.item_count ?? '-'} items</span>
